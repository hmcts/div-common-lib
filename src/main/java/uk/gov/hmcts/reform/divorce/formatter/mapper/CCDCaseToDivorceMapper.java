...
import uk.gov.hmcts.reform.divorce.formatter.strategy.reasonfordivorce.ReasonForDivorceContext;
...
import static uk.gov.hmcts.reform.divorce.formatter.mapper.MappingCommons.toYesNoNeverPascalCase;
...
//    @Value("#{${court.details}}")
    ...
    @Mapping(source = "decreeAbsoluteEligibleFromDate",
        dateFormat = SIMPLE_DATE_FORMAT, target = "decreeAbsoluteEligibleFromDate")
    @Mapping(source = "dateRespondentEligibleForDA",
            ...
            .ifPresent(uploadedFiles -> {
                divorceSession.setMarriageCertificateFiles(uploadedFiles.stream()
                    .filter(uploadedFile -> {
                        ...
                        if (!fileIdExists) {
                            log.warn(
                                "Missing uploaded file properties in Case ID: {} - skipping file",
                                caseData.getD8caseReference()
                            );
                        }
                        return fileIdExists;
                    })
